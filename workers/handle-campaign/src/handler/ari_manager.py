...
class ARI:
    ...
    def __init__(self, user=None, password=None, host=None, port=None):
        ...
        self.password = password if password is not None else \
            os.getenv('ASTERISK_PASS', '5_MeO_DMT')
    ...
    def post(self, route, payload=None, headers=None):
        ...
        response = requests.post(uri, auth=(self.user, self.password), json=payload,
                                 headers=headers)
    ...
    def get(self, route):
        ...
        try:
            ...
            logging.error(f"GET Error parsing JSON: {response.text}, "
                          f"Status Code: {response.status_code}")
            return response
    ...
    def start_channel_recording(self, channel_id, name, format, maxDurationSeconds=0,
                                maxSilenceSeconds=0, ifExists='fail', beep=False,
                                terminateOn='none'):
